...
const Responder = () => {
  ...
  useEffect(() => {
    ...
    socket.on('sos_status_update', ({ sosId, status, responderId }) => {
      setAlerts((prev) =>
        prev.map((a) =>
          a._id === sosId ? { ...a, status, assignedTo: responderId } : a
        )
      );
    });
  ...
  const updateStatus = (id, newStatus) => {
    setAlerts((prev) =>
      prev.map((a) =>
        a._id === id ? { ...a, status: newStatus, assignedTo: myId } : a
      )
    );
  ...
  const myAlerts = alerts.filter(
    (a) =>
      a.status !== 'resolved' &&
      (a.status === 'pending' || a.assignedTo === myId)
  );
      ...
      <div className="w-1/2 flex flex-col border-r border-gray-800">
        <div className="p-6 border-b border-gray-800 bg-gray-800 z-10">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-3xl font-extrabold text-red-400 flex items-center gap-3">
              <ShieldAlert size={30} /> DISPATCH
            </h1>
            <button
              onClick={handleLogout}
              className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition"
            >
          ...
          <div className="flex justify-between items-center mt-2">
            <p className="text-gray-400 text-sm flex items-center gap-1">
              <User size={14} /> Officer: {localStorage.getItem('userName')}
            </p>
        ...
        {activeSOSId && myProfile && (
          <motion.div
            ...
            className="bg-blue-900/50 border-b border-blue-700 p-4"
          >
              ...
              {myLocation && (
                <div className="text-right">
                  <p className="text-lg font-extrabold text-white">
                    {calculateEtaDisplay(
                      alerts.find((a) => a._id === activeSOSId)?.location
                    )}
                  </p>
                  <p className="text-xs text-blue-300">ETA / DISTANCE</p>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-4 text-xs text-gray-300 mt-2">
              ...
              <span className="flex items-center gap-1">
                <Truck size={14} /> {myProfile.vehicleNumber}
              </span>
            </div>
          </motion.div>
        )}
          ...
          <AnimatePresence>
            {myAlerts.length === 0 && (
              <div className="text-center text-gray-500 mt-10">
                All clear. Standing by for emergencies.
              </div>
            )}
            ...
            {myAlerts.map((alert) => (
              <motion.div
                ...
                className={`p-5 rounded-xl border-l-4 shadow-xl transition-all ${
                  alert.status === 'assigned'
                    ? 'border-blue-500 bg-gray-800 hover:bg-gray-700/70'
                    : 'border-red-500 bg-gray-800 hover:bg-gray-700/70'
                }`}
              >
                {alert.image && (
                  <div className="mb-3 rounded-lg overflow-hidden border border-gray-700 relative group h-40">
                    <img
                      src={alert.image}
                      alt="Evidence"
                      className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                    />
                    <div className="absolute bottom-0 left-0 w-full bg-black/60 p-2 text-xs text-white font-bold backdrop-blur-sm">
                      📸 ATTACHED EVIDENCE
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-sm mb-4 bg-gray-900 p-2 rounded-lg border border-gray-700">
                  <span className="flex items-center gap-2 text-red-300 font-bold">
                    <Target size={16} /> MISSION TARGET:
                  </span>
                  <span className="text-yellow-400 font-bold">
                    {calculateEtaDisplay(alert.location)}
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  {alert.status === 'pending' && (
                    <motion.button
                      ...
                      onClick={() => updateStatus(alert._id, 'assigned')}
                      className="flex-1 bg-red-600 hover:bg-red-700 py-3 rounded-lg font-bold transition flex items-center justify-center gap-2"
                    >
                      <Navigation size={18} /> ACCEPT & TRACK
                    </motion.button>
                  )}
                  ...
                  {alert.status === 'assigned' && (
                    <>
                      <motion.button
                        ...
                        className="bg-yellow-500 hover:bg-yellow-600 text-black px-4 rounded-lg font-bold flex items-center justify-center gap-2"
                        title="Open Maps"
                      >
                      ...
                      <motion.button
                        ...
                        className="flex-1 bg-blue-600 py-3 rounded-lg font-bold flex items-center justify-center gap-2"
                      >
                      ...
                      <motion.button
                        ...
                        className="px-4 bg-green-600 hover:bg-green-700 rounded-lg font-bold"
                      >
                        <CheckCircle size={20} />
                      </motion.button>
                    </>
          ...
          {myAlerts.map(
            (alert) =>
              alert.location && (
                ...
                >
                  <Popup>
                    <b className="text-red-600">{alert.description}</b>
                  </Popup>
                </Marker>
              )
          )}
