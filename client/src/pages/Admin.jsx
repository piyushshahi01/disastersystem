...
import {
  Bell, Map, Users, AlertTriangle, Shield, Home, Target, Plus, Trash2, Phone,
  BarChart3, Edit3, X, LogOut, MessageSquare, Flame, CheckCircle
} from 'lucide-react';
...
const ShelterEditModal = ({ shelter, onClose, onSave, onDelete }) => {
  ...
  const handleDeleteClick = () => {
    if (window.confirm(`Delete ${shelter.name}?`)) { onDelete(shelter.id); onClose(); }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[500] font-sans">
      <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-gray-800 p-6 rounded-xl w-96 shadow-2xl border border-gray-700">
        <h3 className="text-xl font-bold text-blue-400 mb-4 flex justify-between items-center">
          Edit Shelter <button onClick={onClose}><X size={20} /></button>
        </h3>
        ...
        <div className="flex justify-between">
          <button onClick={handleDeleteClick} className="bg-red-600 px-4 py-2 rounded">Delete</button>
...
const Admin = () => {
  ...
  useEffect(() => {
    ...
    socket.on('new_shelter', (data) =>
      setShelters(prev => [...prev, { id: data._id, name: data.name, lat: data.lat, lng: data.lng, capacity: data.capacity }])
    );
  ...
  const handlePublishWarning = () => {
    if (!broadcastMsg || !warningLocation) return alert("Select location & type message!");
    ...
    <div className="flex h-screen bg-gray-900 text-white font-sans relative">
      ...
      <div className="w-72 bg-gray-800 shadow-2xl z-10 flex flex-col border-r border-gray-700">
        ...
        <div className="p-4 border-t border-gray-700">
          <button onClick={handleLogout} className="w-full bg-red-600/20 text-red-400 font-bold p-2 rounded-lg hover:bg-red-600/30 transition flex items-center justify-center gap-2">
        ...
        <div className="flex-1 p-6 overflow-hidden">
            ...
            <div className="flex gap-6 h-full">
              <div className="flex-2 w-2/3 bg-gray-800 rounded-2xl shadow-xl border border-gray-700 overflow-hidden relative">
                <MapContainer center={[28.6139, 77.2090]} zoom={12} style={{ height: '100%', width: '100%' }}>
                  ...
                  {showHeatmap ? (
                    ...
                    <>
                      {/* Warning Circle */}
                      {warningLocation && (
                        <Circle center={warningLocation} radius={1000} pathOptions={{ color: 'red', fillColor: 'red', fillOpacity: 0.4 }} />
                      )}
                      ...
                      {shelters.map(s =>
                        s.lat && s.lng ? (
                          <Marker key={s.id} position={[s.lat, s.lng]}>
                            <Popup>🏠 {s.name} (Cap: {s.capacity})</Popup>
                          </Marker>
                        ) : null
                      )}
                          ...
                          <Marker key={a._id} position={[a.location.lat, a.location.lng]}>
                            <Popup>
                              <div className="text-center min-w-[150px]">
                                <strong className={a.type === 'EMERGENCY' ? 'text-red-600' : 'text-blue-600'}>
                                  {a.type === 'EMERGENCY' ? '🚨 SOS ALERT' : '📝 INCIDENT REPORT'}
                                </strong>
                                <p className="text-sm my-1">{a.description}</p>
                                <span className={`text-xs px-2 py-1 rounded text-white ${a.status === 'assigned' ? 'bg-blue-500' : 'bg-red-500'}`}>
                                  Status: {a.status}
                                </span>
                                {a.image && (
                                  <div className="mt-2 border border-gray-300 rounded overflow-hidden">
                                    <img src={a.image} alt="Evidence" className="w-full h-32 object-cover" />
                                  </div>
                                )}
                                <button onClick={() => setActiveChatSosId(a._id)} className="mt-2 bg-blue-600 text-white px-3 py-1 rounded text-xs flex items-center gap-1 mx-auto">
                ...
                <div className="absolute top-4 right-4 z-[1000] bg-gray-900/90 p-2 rounded-lg shadow-xl border border-gray-700">
                  <button onClick={() => setShowHeatmap(!showHeatmap)} className={`flex items-center gap-2 px-3 py-2 rounded font-bold text-xs transition ${showHeatmap ? 'bg-orange-600 text-white' : 'bg-gray-700 text-gray-300'}`}>
              ...
              <div className="flex-1 w-1/3 flex flex-col gap-6">
                <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700">
                  ...
                  <>
                    <button onClick={() => setMapMode('warning')} className={`w-full mb-3 p-2 rounded border-2 ${mapMode === 'warning' ? 'border-red-500 text-red-400' : 'border-gray-700 text-gray-400'}`}>
                    ...
                    <button onClick={handlePublishWarning} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold p-2 rounded">
                      3. PUBLISH WARNING
                    </button>
                  </>
                </motion.div>
                ...
                <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700">
                  ...
                  <button onClick={() => setMapMode('shelter')} className={`w-full p-3 rounded border-2 font-bold flex items-center justify-center gap-2 ${mapMode === 'shelter' ? 'border-green-500 text-green-400' : 'border-gray-700 text-gray-400'}`}>
          ...
          {activeChatSosId && (
            <div className="absolute bottom-4 right-4 z-[2000]">
              <ChatWindow socket={socket} sosId={activeChatSosId} userName="Admin HQ" role="admin" />
            </div>
          )}
                ...
                <tbody>
                  {shelters.map(s => (
                    <tr key={s.id} className="border-b border-gray-700 hover:bg-gray-700/50 transition">
                      ...
                      <td className="p-3 text-gray-500">{s.lat ? s.lat.toFixed(4) : 0}, {s.lng ? s.lng.toFixed(4) : 0}</td>
                      <td className="p-3 text-yellow-400">{s.capacity}</td>
                      <td className="p-3">
                        <button onClick={() => setEditingShelter(s)} className="text-blue-500 hover:text-blue-400 transition">
                          <Edit3 size={16} />
                        </button>
                        <button onClick={() => handleDeleteShelter(s.id)} className="text-red-500 hover:text-red-400 ml-3 transition">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </tr>
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {realResponders.map(r => (
                    <tr key={r._id} className="border-b border-gray-700 hover:bg-gray-700/50 transition">
                      ...
                      <td className="p-3 text-gray-500">{r.vehicleNumber || "N/A"}</td>
                      <td className="p-3">
                        <span className="bg-green-600/30 text-green-400 px-2 py-1 rounded text-xs font-bold">Ready</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
const SidebarItem = ({ icon, label, active, onClick }) => (
  <motion.div whileHover={{ x: 5 }} onClick={onClick} className={`p-3 rounded-lg cursor-pointer flex items-center gap-3 ${active ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`}>
    {icon} {label}
  </motion.div>
);
