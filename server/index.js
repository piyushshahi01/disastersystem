...
// ✅ frontend URLs (deployed + local dev)
const FRONTEND_ORIGINS = [
  'https://disastersystem.vercel.app',
  'http://localhost:5173',
];
...
io.on('connection', (socket) => {
      ...
      if (user.emergencyContacts && user.emergencyContacts.length > 0) {
        ...
        user.emergencyContacts.forEach((c) =>
          sendMockSMS(
            c.name,
            c.phone,
            `SOS! ${user.name} needs help. Location: ${mapLink}`
          )
        );
      }
    }
  ...
  socket.on('get_missing_people', async () => {
    try {
      const list = await MissingPerson.find({ status: 'approved' }).sort({
        createdAt: -1,
      });
  ...
  socket.on('delete_alert', () => {
    io.emit('clear_alerts');
  });
  ...
  socket.on('responder_location_update', (data) => {
    io.emit('live_responder_location', data);
  });
});
